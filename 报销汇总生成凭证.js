...
if ((fyrow+0) > 0) {
  ...
  for (var i = 1; i <= fyrow; i++) {
    ...
    debugger;
    console.log(zhaiyaovar);
    let zhaiyaoV = "支付"+(zhaiyaovar && zhaiyaovar.length > 0 && zhaiyaovar[0] && zhaiyaovar[0].name) ? zhaiyaovar[0].name : '' +"报销款";

    const bxjeV = parseFloat(curformsdk.getFieldValue(`${bxjeFieldMark}_${curFeiYongRowId}`) || 0)||0;

    resultMap[zhaiyaoV] = toFixedNum((resultMap[zhaiyaoV] || 0) + bxjeV );
